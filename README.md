# Manoja Agencies website — current build (plain HTML/CSS/JS)

This is the **current, active version** of the site. It replaces the Astro-based
approach described further down this file (kept below for history — that build was
never deployed and this repo now ships a different, simpler stack).

**Manoja Agencies (मनोजा एजन्सी)** is an authorised dealer of Waaree Energies
(solar panels, inverters, cables, accessories) based in Latur, Maharashtra. This
is a lead-generation site: WhatsApp / call / enquiry form are the only conversions
that matter.

## Stack

- Plain **HTML5 + CSS3 + vanilla JavaScript** — no build step, no framework, no
  npm dependency tree to install for the site itself.
- **Three.js** (loaded from a CDN) for the rotating-globe hero animation only.
- **PHP** (`send-enquiry.php`) as the contact-form handler, for Hostinger shared
  hosting — emails the lead, no database.
- Bilingual: Marathi (default) / English, toggled client-side via `data-lang` on
  `<html>` and `.mr` / `.en` span pairs throughout the markup.

## Pages

| File | Purpose |
| --- | --- |
| `index.html` | Home — hero with rotating 3D globe, stats, "What We Sell" showcase, CTA |
| `products.html` | Full catalogue: panels, inverters, cables, accessories, each with variant grids |
| `contact.html` | Address (showroom + warehouse), hours, map embeds, enquiry form |
| `send-enquiry.php` | Mail handler for the enquiry form |

## Structure

```
index.html, products.html, contact.html, send-enquiry.php
css/style.css            single stylesheet, CSS custom properties for theme
js/
  main.js                 language toggle, mobile nav
  earth.js                 rotating globe (Three.js): Earth texture, land/ocean
                            detection via canvas pixel sampling, solar-panel tiles
                            on ocean, tree sprites on land, Latur/Maharashtra pin
  counters.js              stat number count-up animation
  contact-form.js          enquiry form submit handler (fetch → send-enquiry.php)
assets/                    SVG category illustrations
assets/products/           real product/showroom photos
```

## Running locally

No install needed — it's static files plus a PHP endpoint.
...
The enquiry form posts to `send-enquiry.php`, which needs a PHP runtime to actually
send mail (won't work under `python -m http.server` — fine for everything else on
the site, including the WhatsApp/call CTAs which are plain links).

## Deploying

Upload the files as-is to Hostinger `public_html/` (or any shared PHP host). No
build step. Confirm PHP's `mail()` is deliverable from the host (SPF/DKIM for the
sending domain) before relying on the enquiry form in production — untested as of
this commit.

## Content rules this build follows

- No invented marketing copy — every claim on the site traces back to a fact the
  business owner gave directly (dealer status, service area, stats, hours).
- No prices anywhere — pricing is WhatsApp/call only.
- Service area is **Latur only** — never implied wider.
- "Authorised dealer" of Waaree Energies — not "authorised stock" or similar.

---

# (Below: original README from an earlier Astro-based approach, kept for history)
